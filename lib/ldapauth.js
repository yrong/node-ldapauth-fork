  ...
  self._adminBind(function(bindErr) {
    ...
    self._adminClient.search(searchBase, options, function(searchErr, searchResult) {
      if (searchErr) {
        ...
        var items = [],page=1
        options.paged = options.paged || {"pageSize": 50, "page":1}
      searchResult.on('searchEntry', function(entry) {
          if(page == options.paged.page) {
        ...
        searchResult.on('page', function(result,cb) {
            if(page == options.paged.page){
...
LdapAuth.prototype.authenticate = function(username, password, callback) {
  ...
  self._findUser(username, function(findErr, user) {
    if (findErr) {
        error_message = `ldap authenticate: root bind error: ${String(findErr)}`;
        self.log && self.log.error(error_message);
        return callback(new Error(error_message));
    } else if (!user) {
        error_message = `ldap authenticate: find user ${username} error,no such user`;
        self.log && self.log.error(error_message);
        return callback(new Error(error_message));
    }
    ...
    self._userClient.bind(user[self.opts.bindProperty], password, function(bindErr) {
      if (bindErr) {
          error_message = `ldap authenticate: user login error: ${String(bindErr)}`;
          self.log && self.log.error(error_message);
          return callback(new Error(error_message));
      }
      ...
      self._getGroups(user, function(groupErr, userWithGroups) {
        if (groupErr) {
            error_message = `ldap authenticate: group search error ${String(groupErr)}`
            self.log && self.log.error(error_message);
            return callback(new Error(error_message));
        }
        if (self.opts.cache) {
          bcrypt.hash(password, self._salt, function(bcryptErr, hash) {
            if (bcryptErr) {
                error_message = `ldap authenticate: bcrypt error, not caching ${String(bcryptErr)}`
                self.log && self.log.error(error_message);
                return callback(new Error(error_message));
            } else {
